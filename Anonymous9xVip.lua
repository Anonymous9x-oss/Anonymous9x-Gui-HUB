...
local FIREBASE_URL = "https://loginvipanonymous9x-default-rtdb.asia-southeast1.firebasedatabase.app/" -- GANTI DENGAN URL ANDA
...
-- ==================== FUNGSI DEVICE ID (UNIK PER INSTALASI ROBLOX) ====================
local function GetDeviceID()
    local clientId = Analytics:GetClientId()
    local salt = "ANONYMOUS9x_SALT"
    local combined = clientId .. salt
    if syn and syn.crypt and syn.crypt.hash then
        return syn.crypt.hash(combined, "sha256")
    else
        return clientId
    end
end
...
local function GetAutoLoginUser()
    ...
    local data = FirebaseRequest("GET", "device_to_user/" .. deviceId)
...
local function RegisterDevice(username, deviceId)
    ...
    userData.devices[deviceId] = os.time()
    
    local ok1 = FirebaseRequest("PUT", userPath, HttpService:JSONEncode(userData))
    local ok2 = FirebaseRequest("PUT", "device_to_user/" .. deviceId, HttpService:JSONEncode(username))
...
LogBtn.MouseButton1Click:Connect(function()
    ...
    local deviceId = GetDeviceID()
    local regSuccess, regError = RegisterDevice(checkUsername, deviceId)
    
    if not regSuccess and regError == "MAX_DEVICES" then
        ErrorSound:Play()
        LogBtn.Text = "DEVICE LIMIT"
        LogBtn.BackgroundColor3 = Color3.fromRGB(180, 0, 0)
        task.wait(2)
        LogBtn.Text = "AUTHENTICATE"
        LogBtn.BackgroundColor3 = Color3.new(1,1,1)
...
print(">> [ANONYMOUS9x VIP Login Panel] Loaded with Server Auto-Login (max 2 devices)")
print(">> [INFO] Jika sudah pernah login, lain kali akan langsung masuk tanpa panel login.")
